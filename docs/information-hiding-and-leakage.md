...
*Рис. 5.1: POST-запрос в протоколе HTTP — это просто текст, отправленный через TCP-сокет. Первая строка содержит тип запроса (POST для отправки форм), URL с параметрами (photo_id=246), и версию протокола. Дальше идут заголовки (Content-Length и т.д.), пустая строка и тело запроса.*
